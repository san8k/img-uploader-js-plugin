...
function upload(selector) {
  ...
  var onUpload = options.onUpload || function () {};

  var input = document.querySelector(selector);
  var preview = createElement('div', ['preview']);
  var openBtn = createElement('button', ['btn'], 'Open');
  var uploadBtn = createElement('button', ['btn', 'primary'], 'Upload');
  ...
  var inputChangeHandler = function inputChangeHandler(e) {
    ...
    files = Array.from(e.target.files); // clear previous files
  ...
  var removeHandler = function removeHandler(e) {
    ...
    var current = preview.querySelector("[data-name=\"".concat(name, "\"]")).closest('.preview-image');
  ...
  var uploadHandler = function uploadHandler(e) {
    preview.querySelectorAll('.preview-remove').forEach(function (o) {
      return o.remove();
    });
    var prevInfo = preview.querySelectorAll('.preview-info');
...
function factory(container, _a) {
  ...
  var storageServiceCompat = new StorageServiceCompat(app, new StorageService(app, authProvider, appCheckProvider, new XhrIoPool(), url, _app.default.SDK_VERSION));
  return storageServiceCompat;
}
...
},{"@firebase/storage":"node_modules/@firebase/storage/dist/index.esm.js"}],"js/app.js":[function(require,module,exports) {
"use strict";
...
var options = {
  ...
  onUpload: function onUpload(files, previews) {
    files.forEach(function (file, i) {
      var ref = storage.ref("/img/".concat(file.name));
      var task = ref.put(file);
      task.on('state_changed', function (snapshot) {
        var percentage = (snapshot.bytesTransferred / snapshot.totalBytes * 100).toFixed(0);
        var prev = previews[i].querySelector('.preview-info-progress');
